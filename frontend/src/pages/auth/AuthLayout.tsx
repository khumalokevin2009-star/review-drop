...
export function AuthLayout({
  ...
  return (
    <div className="min-h-screen bg-[#08090A] font-sans text-white antialiased selection:bg-white/20 lg:grid lg:grid-cols-[1.05fr_1fr]">
      {/* branded panel */}
      <aside
        className={cn("hidden flex-col justify-between border-r p-10 lg:flex", HAIRLINE)}
        style={{
          backgroundImage:
      ...
      <main className="flex min-h-screen items-center justify-center px-6 py-12 lg:min-h-0">
        <div className="w-full max-w-sm">
          <Link
            to="/"
            aria-label="Orvelle home"
            className={cn("mb-10 inline-block rounded-sm lg:hidden", authFocusRing)}
          >
            <Logo size="md" />
          </Link>
          ...
          {footer ? (
            <div className={cn("mt-8 border-t pt-6 text-sm text-[#A1A1AA]", HAIRLINE)}>
              {footer}
            </div>
          ) : null}
        </div>
